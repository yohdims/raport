...
class Kompetensi_Keahlian extends CI_Controller {
	var $hak_akses= "admin";
	var $judul='Kompetensi Keahlian';
	var $link='kompetensi_keahlian';
	...
	public function index($id)
	{
		...
		$this->data['all']=$this->M_Kompetensi_Keahlian->getAll();
		...
		if($id==0){	
			...
			$this->data['satuan']=$this->M_Kompetensi_Keahlian->getID($id);
		}
	...
	public function input()
	{
		...
		if($id==0){	
			if($this->M_Kompetensi_Keahlian->insert($data)){
				$this->session->set_flashdata('message', 'Berhasil tambah data');
			}
		}elseif ($this->M_Kompetensi_Keahlian->update($data)) {// success
	...
	{
		$this->M_Kompetensi_Keahlian->delete($id);
		redirect($this->hak_akses.'/'.$this->link.'/index/0');
	}
