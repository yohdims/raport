...
class Kelas_Tahunan extends CI_Controller {
	var $hak_akses= "admin";
	var $judul='Kelas Tahunan';
	var $link='kelas_tahunan';
	...
	public function index($id)
	{
		...
		$this->data['all']=$this->M_Kelas_Tahunan->getAll();
	...
	public function input()
	{
		...
		if($id==0){	
			if($this->M_Kelas_Tahunan->insert($data)){
				$this->session->set_flashdata('message', 'Berhasil tambah data');
			}
		}elseif ($this->M_Kelas_Tahunan->update($data)) {// success
	...
	{
		$this->M_Kelas_Tahunan->delete($id);
		redirect($this->hak_akses.'/'.$this->link.'/index/0');
	}
