...
class Jenis_Nilai extends CI_Controller {
	var $hak_akses= "admin";
	var $judul='Jenis Nilai';
	var $link='jenis_nilai';
	...
	public function index($id)
	{
		...
		$this->data['all']=$this->M_Jenis_Nilai->getAll();
		if($id==0){	
			$this->data['title2']='Tambah Data '.$this->judul;
		}else{
			$this->data['title2']='Edit Data '.$this->judul;
			$this->data['satuan']=$this->M_Jenis_Nilai->getID($id);
		}
	...
	public function input()
	{
		...
		if($id==0){	
			if($this->M_Jenis_Nilai->insert($data)){
				$this->session->set_flashdata('message', 'Berhasil tambah data');
			}
		}elseif ($this->M_Jenis_Nilai->update($data)) {// success
