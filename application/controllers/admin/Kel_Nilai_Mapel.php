...
class Kel_Nilai_Mapel extends CI_Controller {
	var $hak_akses= "admin";
	var $judul='Kel Nilai Mapel';
	var $link='kel_nilai_mapel';
	...
	public function index($id)
	{
		...
		$this->data['all']=$this->M_Kel_Nilai_Mapel->getAll();
		if($id==0){	
			$this->data['title2']='Tambah Data '.$this->judul;
		}else{
			$this->data['title2']='Edit Data '.$this->judul;
			$this->data['satuan']=$this->M_Kel_Nilai_Mapel->getID($id);
		}
	...
	public function input()
	{
		...
		if($id==0){	
			if($this->M_Kel_Nilai_Mapel->insert($data)){
				$this->session->set_flashdata('message', 'Berhasil tambah data');
			}
		}elseif ($this->M_Kel_Nilai_Mapel->update($data)) {// success
	...
	{
		$this->M_Kel_Nilai_Mapel->delete($id);
		redirect($this->hak_akses.'/'.$this->link.'/index/0');
	}
