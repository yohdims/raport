...
class Program_Keahlian extends CI_Controller {
	var $hak_akses= "admin";
	var $judul='Program Keahlian';
	var $link='program_keahlian';
	...
	public function index($id)
	{
		...
		$this->data['all']=$this->M_Program_Keahlian->getAll();
		if($id==0){	
			$this->data['title2']='Tambah Data '.$this->judul;
		}else{
			$this->data['title2']='Edit Data '.$this->judul;
			$this->data['satuan']=$this->M_Program_Keahlian->getID($id);
		}
	...
	public function input()
	{
		...
		if($id==0){	
			if($this->M_Program_Keahlian->insert($data)){
				$this->session->set_flashdata('message', 'Berhasil tambah data');
			}
		}elseif ($this->M_Program_Keahlian->update($data)) {// success
	...
	{
		$this->M_Program_Keahlian->delete($id);
		redirect($this->hak_akses.'/'.$this->link.'/index/0');
	}
